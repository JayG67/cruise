...
test.describe('React default mobile replacement checks', () => {
  ...
  test('keeps admin-only React tools available when admin is selected', async ({ page }) => {
    ...
    await expect(page.getByTestId('react-operations-intelligence-center')).toBeVisible({ timeout: 20000 })
  ...
  test('keeps React ship and sailing controls reachable on mobile', async ({ page }) => {
    ...
    await expect(page.getByTestId('react-itinerary-panel')).toBeVisible({ timeout: 20000 })
    await expectNoHorizontalOverflow(page)
  })
  ...
  test('keeps React itinerary CRUD controls reachable on mobile', async ({ page }) => {
    ...
    await page.getByTestId('react-view-itinerary-button').first().click()
    await expect(page.getByTestId('react-itinerary-panel')).toBeVisible({ timeout: 20000 })
    await expect(page.getByTestId('react-create-itinerary-day-form')).toBeVisible({ timeout: 20000 })
    await expect(page.getByTestId('react-create-itinerary-activity-form')).toBeVisible({ timeout: 20000 })
    await expect(page.getByTestId('react-update-itinerary-day-button').first()).toBeVisible({ timeout: 20000 })
    await expect(page.getByTestId('react-delete-itinerary-day-button').first()).toBeVisible({ timeout: 20000 })
    await expect(page.getByTestId('react-update-itinerary-activity-button').first()).toBeVisible({ timeout: 20000 })
    await expect(page.getByTestId('react-delete-itinerary-activity-button').first()).toBeVisible({ timeout: 20000 })
    await expectNoHorizontalOverflow(page)
  })
  ...
  test('keeps Operations Intelligence usable on mobile', async ({ page }) => {
    ...
    await expect(intelligencePanel).toBeVisible({ timeout: 20000 })
    await expect(page.getByTestId('react-operations-intelligence-select')).toBeVisible({ timeout: 20000 })
    await expect(page.getByTestId('react-operations-intelligence-detail')).toBeVisible({ timeout: 20000 })
    await expect(page.getByTestId('react-operations-intelligence-priority-list')).toBeVisible({ timeout: 20000 })
    await expect(page.getByTestId('react-operations-intelligence-setup-button')).toBeVisible({ timeout: 20000 })
    await expect(page.getByTestId('react-operations-intelligence-role-button')).toBeVisible({ timeout: 20000 })
    await expect(page.getByTestId('react-operations-intelligence-refresh-button')).toBeVisible({ timeout: 20000 })
    await expectNoHorizontalOverflow(page)
  })
