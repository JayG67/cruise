...
describe('Cruise validation schemas', () => {
  ...
  describe('bookingSchema', () => {
    ...
    it('normalizes supported booking status values through shared reference data', () => {
      const result = bookingSchema.safeParse({
        ...validBookingPayload,
        bookingStatus: ' paid in full '
      })

      expect(result.success).toBe(true)
      expect(result.data.bookingStatus).toBe('PAID_IN_FULL')
    })

    it('rejects unsupported booking status values before persistence', () => {
      const result = bookingSchema.safeParse({
        ...validBookingPayload,
        bookingStatus: 'almost booked'
      })
