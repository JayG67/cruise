...
describe('React migration scaffold', () => {
  ...
  it('uses React state for the first migration candidate instead of direct DOM selectors', () => {
    ...
    expect(viewStateHook).toContain('useState')
    expect(viewStateHook).toContain('expandedCustomerIds')
    expect(viewStateHook).toContain('expandedBookingIds')
    expect(viewStateHook).toContain('buildCustomerBookingRows')
    expect(component).toContain('useAdminHierarchyViewState')
  ...
  it('extracts React hierarchy domain logic away from component rendering', () => {
    ...
    expect(viewStateHook).toContain('buildCustomerBookingRows')
    expect(viewStateHook).toContain('filterCustomerBookingRows')
  ...
  it('extracts Stage 3 React expansion state transitions from the hierarchy component', () => {
    ...
    expect(viewStateHook).toContain("from '../domain/hierarchyExpansionState.js'")
    expect(viewStateHook).toContain('createBookingExpansionKey(customerId, bookingId)')
...
describe('React migration Stage 15 hierarchy view-state hook extraction', () => {
  const projectRoot = path.resolve(__dirname, '../..')
  const packageJson = JSON.parse(fs.readFileSync(path.join(projectRoot, 'package.json'), 'utf8'))

  it('extracts hierarchy search, summary, and expansion orchestration out of the hierarchy component', () => {
    const hierarchy = fs.readFileSync(path.join(projectRoot, 'frontend/react/src/components/CustomerBookingHierarchy.jsx'), 'utf8')
    const viewStateHook = fs.readFileSync(path.join(projectRoot, 'frontend/react/src/hooks/useAdminHierarchyViewState.js'), 'utf8')
    ...
    expect(viewStateHook).toContain('export function useAdminHierarchyViewState')
    expect(viewStateHook).toContain('buildCustomerBookingRows(customers, bookings)')
    expect(viewStateHook).toContain('filterCustomerBookingRows(allRows, searchTerm)')
    expect(viewStateHook).toContain('summarizeHierarchyRows(rows)')
    expect(viewStateHook).toContain('toggleBooking(customerId, bookingId)')
