...
} = require('../../services/turnaroundApplicationDossier.service')
...
const baseInput = {
  ...
  reviewerPacket: { readiness: { readinessScore: 95 } },
  outreachBoard: { readiness: { readinessScore: 93 } },
  ...
  productionReadiness: { productionScore: 93, productionStatus: 'PRODUCTION_DEMO_READY' }
}

describe('turnaround application dossier service', () => {
  it('builds reviewer-ready application dossier evidence from production readiness inputs', () => {
    const dossier = buildTurnaroundApplicationDossier(baseInput)

    expect(dossier.dossierScore).toBeGreaterThanOrEqual(90)
    expect(dossier.dossierStatus).toBe('APPLICATION_READY')
    expect(dossier.summary).toContain('Gallagher Cruise Labs')
    expect(dossier.reviewerNarrative.headline).toContain('application dossier')
    expect(dossier.evidenceSections.map(section => section.id)).toEqual([
      'production-demo-proof',
      'workflow-proof',
      'reviewer-proof',
      'resilience-proof',
      'audit-proof'
    ...
    expect(dossier.nextApplicationSteps.map(step => step.id)).toContain('expand-cypress-lifecycle')
  })

  it('surfaces watch items when operational proof is not complete', () => {
    const dossier = buildTurnaroundApplicationDossier({
      ...baseInput,
    ...
    expect(dossier.dossierStatus).toBe('NEEDS_PROOF_HARDENING')
    expect(dossier.checklist.filter(item => !item.complete).length).toBeGreaterThan(0)
    expect(dossier.nextAction).toContain('Close watch items')
    expect(dossier.nextApplicationSteps.find(step => step.id === 'close-watch-items').priority).toBe('HIGH')
  })

  it('normalizes raw inputs into dossier proof metrics', () => {
    const inputs = buildDossierInputs(baseInput)
