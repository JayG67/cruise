...
describe('GitHub Pages coverage report preparation', () => {
  it('publishes a flat per-file coverage index while preserving the grouped Istanbul report', () => {
    ...
    expect(script).toContain("const coverageEvidenceJson = path.join(coverageDir, 'coverage-evidence.json')")
    expect(script).toContain("const coverageEvidenceMarkdown = path.join(coverageDir, 'coverage-evidence.md')")
    expect(script).toContain("const coverageEvidenceCsv = path.join(coverageDir, 'coverage-evidence.csv')")
    ...
    expect(script).toContain("[coverageEvidenceJson, 'coverage-evidence.json']")
    expect(script).toContain("[coverageEvidenceMarkdown, 'coverage-evidence.md']")
    expect(script).toContain("[coverageEvidenceCsv, 'coverage-evidence.csv']")
  ...
  it('keeps the CI coverage command generating the detailed inputs required by the published report', () => {
    ...
    expect(packageJson.scripts['coverage:evidence']).toBeUndefined()
  })
  ...
  it('verifies complete raw and published coverage artifacts before GitHub upload', () => {
    ...
    expect(workflow).toContain('Build complete coverage evidence manifest')
    ...
    expect(verifier).toContain("'coverage-evidence.json'")
    expect(verifier).toContain("'coverage-evidence.md'")
    expect(verifier).toContain("'coverage-evidence.csv'")
    const evidenceBuilder = read('scripts/build-coverage-evidence.js')
    expect(evidenceBuilder).toContain("const outputCsv = path.join(coverageDir, 'coverage-evidence.csv')")
    expect(evidenceBuilder).toContain('const projectMarker = `/${path.basename(projectRoot)}/`')
    expect(evidenceBuilder).toContain('path.basename(normalized)')
    expect(verifier).toContain("path.join('lcov-report', 'index.html')")
    expect(jestConfig).toContain('coverageReporters')
    expect(jestConfig).toContain("'cobertura'")
    expect(jestConfig).toContain("'clover'")
    expect(jestConfig).toContain('coverageThreshold')
    expect(jestConfig).toContain('statements: 90.5')
    expect(jestConfig).toContain('branches: 65.5')
    expect(jestConfig).toContain('functions: 94.5')
    expect(jestConfig).toContain('lines: 92.25')
  })
