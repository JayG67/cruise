...
} = require('../../services/turnaroundReviewerPacket.service')

describe('turnaroundReviewerPacket service', () => {
  it('builds a cruise-line reviewer packet from executive and operational signals', () => {
    const packet = buildTurnaroundReviewerPacket({
      operation: {
        id: 'turnaround-1',
    ...
    expect(packet.header.title).toContain('Wonder of the Seas reviewer packet')
    ...
    expect(packet.nextSteps[0]).toContain('Resolve the top watch items')
  })

  it('marks high scoring operations ready for cruise-line review', () => {
    const readiness = buildPacketReadiness({
    ...
    expect(readiness.readinessStatus).toBe('READY_FOR_CRUISE_LINE_REVIEW')
  ...
  it('summarizes clean data quality when no operational watch items are open', () => {
    expect(buildReviewerDataQuality({
