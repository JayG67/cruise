...
describe('local test database script guardrails', () => {
  ...
  it('keeps full Jest coverage while avoiding a duplicate integration pass in the default test script', () => {
    ...
    expect(packageJson.scripts.test).toContain('npm run uiTests')
  ...
  it('adds explicit React browser regression scripts before DOM retirement', () => {
    ...
    expect(packageJson.scripts['uiTests:react']).toContain('http://localhost:8000/app-next')
    ...
    expect(packageJson.scripts['react:cutover:audit']).toContain('test:all')
  })


  it('runs both legacy DOM and React browser suites from test all', () => {
    expect(packageJson.scripts['test:all']).toContain('npm run uiTests')
    ...
    expect(packageJson.scripts['test:all']).toContain('browserTests:legacy')
    ...
    expect(packageJson.scripts['react:cutover:audit']).toContain('npm run test:all')
  })


  it('react cutover audit uses the full combined gate', () => {
    expect(packageJson.scripts['react:cutover:audit']).toContain('react:readiness:audit')
    expect(packageJson.scripts['react:cutover:audit']).toContain('test:all')
    expect(packageJson.scripts['test:all']).toContain('browserTests:legacy')
    expect(packageJson.scripts['test:all']).toContain('browserTests:react')
  })
  ...
  it('builds React assets before Playwright suites that include app next', () => {
