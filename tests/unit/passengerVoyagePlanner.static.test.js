...
describe('passenger voyage planner static contracts', () => {
  it('adds a functional passenger-facing voyage planner instead of another development readiness panel', () => {
    ...
    expect(voyagePlanner).toContain('function PassengerVoyagePlanner')
    expect(voyagePlanner).toContain('data-testid="react-passenger-voyage-planner"')
    expect(voyagePlanner).toContain('data-testid="react-voyage-checklist"')
    expect(voyagePlanner).toContain('updatePassengerPreCruiseChecklist')
    expect(voyagePlanner).toContain('data-testid={`react-voyage-checklist-${id}`}')
    expect(voyagePlanner).toContain('Checklist progress is saved to this passenger profile.')
    expect(voyagePlanner).toContain('favoriteItineraryActivitiesByBooking')
    expect(dashboard).toContain("from './passenger/RolePassengerSurface.jsx'")
    expect(passengerSurface).toContain("export { default as PassengerVoyagePlanner } from './PassengerVoyagePlanner.jsx'")
    expect(passengerSurface).not.toContain('function RoleBookingDetails')
    expect(passengerSurface).not.toContain('function RoleBookingCard')
  ...
  it('keeps pre-cruise checklist persistence in the API and database schema', () => {
    ...
    expect(passengerExperienceController).toContain('exports.updatePassengerPreCruiseChecklist')
